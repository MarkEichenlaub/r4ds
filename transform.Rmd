...
The order of operations doesn't work like English. You can't write `filter(flights, month == 11 | 12)`, which you might literally translate into  "finds all flights that departed in November or December". Instead it first evaluates `month == 11`, then puts the resulting vector into ` | 12`, an expression that evaluates to `TRUE`. This returns all flights and doesn't filter any out. This is quite confusing!
